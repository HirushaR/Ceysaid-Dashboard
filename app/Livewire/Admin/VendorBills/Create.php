...
class Create extends Component
{
    ...
    {
        abort_unless(auth()->user()->can('create', VendorBill::class), 403);
        $data=$this->validate(['invoice_id'=>['nullable','exists:invoices,id'],'supplier_id'=>['required','exists:suppliers,id'],'due_date'=>['required','date'],'service_type'=>['required','string','max:255'],'service_details'=>['nullable','string','max:5000'],'notes'=>['nullable','string','max:5000'],'lines'=>['required','array','min:1'],'lines.*.description'=>['required','string','max:2000'],'lines.*.quantity'=>['required','numeric','gt:0'],'lines.*.rate'=>['required','numeric','min:0']]);
