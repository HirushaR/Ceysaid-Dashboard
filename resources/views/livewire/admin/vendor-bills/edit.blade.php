<div class="mx-auto max-w-5xl space-y-6">
    <div class="page-heading">
        <div><p class="eyebrow">Finance / Vendor bills</p><h1>Edit {{ $vendorBill->vendor_bill_number }}</h1><p>Paid LKR {{ number_format($vendorBill->total_paid_amount, 2) }} · the revised total cannot be lower.</p></div>
    ...
    <form wire:submit="save" class="space-y-6">
        <section class="panel p-5">
            <div class="grid gap-5 md:grid-cols-2">
                ...
                <div><label class="form-label">Notes</label><input wire:model="notes" class="form-input"></div>
            ...
            <div class="space-y-3 p-5">
                @foreach($lines as $index => $line)
                    <div class="grid gap-3 rounded-xl bg-slate-50 p-3 md:grid-cols-[1fr_120px_160px_44px] dark:bg-slate-800">
                        <input wire:model="lines.{{ $index }}.description" class="form-input">
